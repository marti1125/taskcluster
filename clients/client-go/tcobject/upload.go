...
// Server with content buf and given projectID, name, contentType and expires.
...
// UploadFromReadSeeker publishes an Object to the Object Server, with given
...
func (object *Object) UploadFromReadSeeker(projectID string, name string, contentType string, contentLength int64, expires time.Time, readSeeker io.ReadSeeker) (err error) {
	...
	proposedUploadMethods.PutURL = PutURLUploadRequest{
		// TODO: find out why this is float64 when it should be int64
	...
	// Ensure FinishUpload is called even if upload fails by placing in a defer
	// block.
	defer func() {
		err2 := object.FinishUpload(
			name,
			&FinishUploadRequest{
				ProjectID: projectID,
				UploadID:  uploadID,
		...
		// only update returned error if it isn't already set
		if err == nil {
			err = err2
		}
	}()
